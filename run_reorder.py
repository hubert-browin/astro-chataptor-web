...
src_path = "c:/Users/hudzi/Desktop/Antigravity/chataptorwebsite/src/pages/[lang]/index.astro"

with open(src_path, "r", encoding="utf-8") as f:
    content = f.read()

UI_MOCKUP_START = '      <!-- UI MOCKUP -->'
FEATURE_BENTO_START = '      <!-- FEATURE BENTO GRID (Features) -->'
EARLY_ADOPTERS_START = '      <!-- EARLY ADOPTERS SECTION -->'
IMPLEMENTATION_START = '      <!-- SEKCJA: WDROŻENIE (Implementation) -->'
HOW_IT_WORKS_START = '      <!-- SEKCJA: ŚCIEŻKA EKSPANSJI (How it works) -->'
PRICING_START = '      <!-- PRICING SECTION -->'

pMockup = content.find(UI_MOCKUP_START)
pFeatures = content.find(FEATURE_BENTO_START)
pEarlyAdopters = content.find(EARLY_ADOPTERS_START)
pImplementation = content.find(IMPLEMENTATION_START)
pHowItWorks = content.find(HOW_IT_WORKS_START)
pPricing = content.find(PRICING_START)

if -1 in (pMockup, pFeatures, pEarlyAdopters, pImplementation, pHowItWorks, pPricing):
    print("Could not find one or more markers.")
    sys.exit(1)

beforeMockup = content[:pMockup]
mockupBlock = content[pMockup:pFeatures]
featuresBlock = content[pFeatures:pEarlyAdopters]
earlyAdoptersBlock = content[pEarlyAdopters:pImplementation]
implementationBlock = content[pImplementation:pHowItWorks]
howItWorksBlock = content[pHowItWorks:pPricing]
pricingAndAfter = content[pPricing:]
...
newHowItWorks = """      <!-- SEKCJA: ŚCIEŻKA EKSPANSJI (How it works) - ENTERPRISE EDITION -->
      <section id="how-it-works" class="py-20 md:py-32 px-4 md:px-8 w-full relative">
        <div class="max-w-[76rem] mx-auto bg-zinc-950 rounded-[3rem] p-8 md:p-16 relative overflow-hidden ring-1 ring-zinc-800 shadow-[0_0_80px_-20px_rgba(0,0,0,0.5)]">
          <!-- Ambient Glows -->
...
newContent = beforeMockup + earlyAdoptersBlock + featuresBlock + implementationBlock + mockupBlock + newHowItWorks + "\\n" + pricingAndAfter
